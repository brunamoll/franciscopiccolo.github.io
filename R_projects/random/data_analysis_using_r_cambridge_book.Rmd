...
# Lag plot para ver autocorrelação das observações com seus próprios valores de períodos passados
lag.plot(LakeHuron, lags = 6, do.lines = F)

# Validação da autocorrelação entre os lags
acf(LakeHuron)
```

2) Use a função 'ar' para ajustar um modelo autoregressivo de segunda ordem no data set Lake Huron.
