...
$\beta_0$ = Intercepto da equação

$\beta_1$ = Inclinação da equação

$U_t$ = Ruído do modelo (variações não explicadas pelo modelo)
...
Esta equação indica que a plantação de cana de açucar seria de $e^{1.64} = 5.15$ hectares caso o preço fosse 0. Conforme o preço aumenta em 1% o impacto na área plantada será de 0.97%, pois ambas as variáveis estão transformadas em log natural, logo uma variação unitária é interpretada como variação percentual (conforme demonstrado acima). 
...
lmtest::dwtest(model) %>% pander::pander()
```
