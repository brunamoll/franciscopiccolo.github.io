...
Para o grupo de controle (trt = 0), a média de renda estimatva com nível de confiança de 95% fica entre 6.267 e 10.589. Para o grupo de teste (trt = 1), a média de renda estimada com nível de confiança de 95% fica entre 5.334 e 9.023. 
...
Esta questão pede o mesmo cálculo acima, porém para os dados de 1975. Vamos fazer o mesmo gráfico e em seguida, plotar em uma única tabela os valores necessários para criação dos intervalos e os intervalos (limite inferior e superior). 
...
DAAG::nsw74demo %>%
  ...
  scale_x_log10(label = scales::comma)+
  theme(legend.title = element_blank())+
  labs(title = "Income distribution between groups (1975)",
       x = "Income, log10 scale",
       y = "")
```
...
DAAG::nsw74demo %>%
  ...
  select(group,
         re78) %>% 
  ggplot(aes(x = re78, fill = group))+
  geom_histogram(alpha = .4)+
  scale_x_log10(label = scales::comma)+
  theme(legend.title = element_blank())+
  labs(title = "Income distribution between groups (1978)",
       x = "Income, log10 scale",
       y = "")
```
...
graph_1 <- t_value %>% 
  ...
  labs(title = "Change in t-statistics by change in df",
...
graph_2 <- t_value %>% 
  ...
  theme(panel.grid = element_blank(),
        axis.text = element_text(size =  axis_size))+
  labs(title = "Change in t-statistics by change in df",
       subtitle = "log transformation not applied",
       x = "Degrees of freedom",
       y = "t-statistics")

gridExtra::grid.arrange(graph_1,graph_2)
```
