...
$$\frac{E}{D+E} = 1-\frac{D}{D+E}   $$
Onde:

$\frac{D}{D+E}$ = Proporção de capital de terceiros

$\frac{E}{D+E}$ = Proporção de capital próprio
...
Seguindo os conceitos de [artigo](http://www.fazenda.gov.br/centrais-de-conteudos/publicacoes/guias-e-manuais/metodologia-de-calculo-do-wacc2018.pdf), para o retorno de títulos com baixo risco pode-se utilizar a taxa Selic, visto que esta taxa é usada nos títulos públicos que são os de menor risco no Brasil. O retorno de mercado pode ser obtido através da taxa de rendimento obtida no mercado acionário, sendo neste caso uma boa alternativa o índice Bovespa [base](http://bvmf.bmfbovespa.com.br/indices/ResumoTaxaMediaCrescimento.aspx?Indice=IBOV&idioma=pt-br).
...
A combinação das duas técnicas ocorre através da identificação das principais variáveis que formam o fluxo de caixa operacional da empresa que podem assumir comportamento estocástico, bem como das variáveis usadas no cálculo do valor residual e da taxa de desconto *WACC*. Após esta definição, é necessário estudar e definir a curva de distribuição de probabilidade para cada variável do modelo que assumirá comportamento estocástico. Tendo posse das variáveis e da distribuição de probabilidade de cada uma, é possível criar um modelo que terá como variável de entrada não apenas um único valor, mas uma grande quantidade de valores que serão gerados aleatoriamente pela técnica de Monte Carlo, com base na respectiva distribuição de probabilidade definida.

Na literatura foi possível identificar diversos autores que realizaram a combinação das duas técnicas, adaptando-as para cada cenário encontrado. Classen (2017) realizou a combinação da Simulação Monte Carlo com o *valution* estimat o valor de uma empresa, visando apoiar o processo decisório de fundos de *Venture* *Capital*. As variáveis indicadas como estocásticas deste modelo foram receita líquida, custos de matéria prima, custos com pessoal, despesas administrativas e despesas com vendas. As distribuições de probabilidade foram determinadas como triangulares, devido à falta de informações históricas para realizar testes mais formais para definir qual distribuição se encaixaria melhor nas as variáveis. 

Já Buratto (2005) utilizou a técnica da Simulação Monte Carlo no estudo de empresas tomadoras de crédito. O objetivo era calcular a probabilidade destas empresas não pagarem a dívida assumida. A principal variável estocástica inserida no modelo foi a receita bruta, que também foi definida como tendo uma distribuição triangular. Neste trabalho houve os testes Kolmogorov-Smirnov, Qui-quadrado e Anderson-Darling, que não apresentaram resultados que permitissem enquadrar as variáveis de entrada do *valuation* em uma determinada distribuição de probabilidade existente.
...
1. Coleta de dados financeiros necessários para o cálculo do Fluxo de Caixa Operacional e da taxa de desconto (WACC).
2. Definição das variáveis que assumirão comportamento estocástico.
3. Definição da distribuição de probabilidade das variáveis estocásticas.
4. Análise e definição de correlações entre as variáveis estocásticas.
5. Criação do algoritmo para realização das iterações e geração do valor da empresa para cada iteração.

O processo será aplicado em x empresas, para que seja possível avaliar se o algoritmo está gerando valores acurados. Para validar a acuracidade do algoritmo, será gerado um gráfico comparando o valor atual da ação da empresa com o valor médio da ação gerado pelo algoritmo, mais ou menos dois desvio padrão, ou seja, uma faixa de valor estimado.

As empresas selecionadas para aplicação do algoritmo foram: Alpargatas, Carrefour, Camil, CPFL Energia, Estácio, Duratex e Fleury. Após coletar os dados financeiros históricos destas empresas, especificamente de 2015 até 2018, e consolidá-los de maneira padronizada, se tornou possível definir quais variáveis deverão assumir comportamento estocástico, bem como qual distribuição de probabilidade melhor se aplica a variável e quais parâmetros foram calculados para a geração dos números aleatórios. 

A tabela abaixo resume estas informações:

```{r,eval=T,include=T,echo=F,warning=F,message=F}
knitr::kable(x = data.frame(
  "Variável" = c("EBTIDA","Investimento","Custo de capital de terceiros","Retorno de mercado","Retorno livre de risco","Parcela de capital próprio","Parcela de capital de terceiros","Beta","Crescimento esperado"),
  Comportamento = c("Estocástica","Estocástica","Determinística","Estocástia","Estocástica","Determinística","Determinística","Determinística","Determinística"),
  "Distribuição" = c("Normal","Normal","","Triangular","Triangular","","","",""),
  "Parâmetros calculados" = c("Média e Desvio Padrão","Média e Desvio Padrão","","Valor minimo, máximo e moda","Valor minimo, máximo e moda","","","","")
))
```

Tendo definida esta etapa, foi gerado um algoritmo para utilizar como insumo as informações coletadas e calcular o possível valor da ação das empresas.

```{r,eval=T,echo=F,include=F,warning=F,message=F}
library(tidyverse) # pacote para tratar os dados
library(triangle) # pacote para gerar distribuição triangular
library(knitr) # pacote para renderizar tabelas num melhor formato
library(scales) # pacote para ajustar escala dos eixos do gráfico
library(googlesheets) # pacote para ler planilhas do Google Drive
library(mvtnorm) # pacote para gerar números aleatórios correlacionados
library(ggalt) # pacote para gerar o gráfico de resultados

th <- theme_light()

# buscando nome das empresas para rodar o loop com o algoritmo
company <- as.character(googlesheets::gs_title("dados_monografia") %>% googlesheets::gs_ws_ls())

df <- NULL

for (k in company){
  dados <- googlesheets::gs_title("dados_monografia") %>% 
    googlesheets::gs_read(ws = k)

  n <- 5
  ano <- dados[1,2:5]
  stock_price <- as.numeric(dados[3,8]) # atual valor da ação
  stock_vol <- as.numeric(dados[2,8])   # número de ações
  g <- as.numeric(dados[5,8])           # crescimento esperado da empresa
  rm <- rtriangle(n=10000,a=0.05,b=0.15,c=0.096)   # retorno do mercado
  rf <- rtriangle(n=10000,a=0.05,b=0.09,c=0.065)   # retorno livre de risco
  beta <- as.numeric(dados[4,8])                   # beta da empresa

  ebitda <- as.numeric(dados[9,2:n])*1000          # ebtida
  avg_ebitda <- mean(ebitda)                       
  sd_ebitda <- sd(ebitda)
  
  invest <- as.numeric(dados[10,2:n])*1000         # investimento
  avg_invest <- mean(invest)
  sd_invest <- sd(invest)
  
  equity <- as.character(dados[13,2:n])            # equity/debt+equity
  equity <- stringr::str_remove_all(equity,"%")
  equity <- str_replace_all(equity,",",".")
  equity <- as.numeric(equity)
  equity <- round(mean(equity),digits = 2)/100
  debt <- 1-equity

  kd <- as.character(dados[12,2:n])
  kd <- stringr::str_remove_all(kd,"%")
  kd <- str_replace_all(kd,",",".")
  kd <- as.numeric(kd)
  avg_kd <- mean(kd)
  sd_kd <- sd(kd)
  kd <- rtriangle(n=10000,a=avg_kd-(2*sd_kd),b=avg_kd+(2*sd_kd),c=avg_kd)/100

  ke <- rf+(beta*(rm-rf))
  wacc <- ((debt)*(kd)) + ((equity*ke))

  # Valor residual
  vr <- (avg_ebitda-avg_invest)/(wacc-g) # Valor Residual

  # Iterações
  iter <- 10000 
  
  saida <- rep(NA,iter)
  for (i in 1:iter)     # criando loop do algoritmo
  {
    saida[i] <- 
      (data.frame(              # criando data.frame para cada iteração
        ebda = rnorm(10, avg_ebitda, sd_ebitda),
        inv = rnorm(10, avg_invest, sd_invest),
        t = 1:10
      )%>%
        mutate(fco = ebda-inv,
               fcd = (fco / (1+(sample(wacc,size = 1)))^t)
        )%>% 
        summarize((sum(fcd)+sample(vr,size = 1))/(stock_vol))) # sumarizando os 10 anos e somando os valores com o valor residual
  }

  dados_saida <- 
    data.frame(
      ev = matrix(unlist(saida),nrow = iter,byrow = T),stringsAsFactors=FALSE)

  df =
      rbind(df,data.frame(
        name = k,
        avg = round(mean(dados_saida$ev),digits = 2),
        max = round(mean(dados_saida$ev)+(2*(sd(dados_saida$ev))),digits = 2),
        min = round(mean(dados_saida$ev)-(2*(sd(dados_saida$ev))),digits = 2),
        real = stock_price
        )
    )
}
...
```{r,eval=T,include=T,warning=F,message=F,echo=F,fig.align='center',fig.width=5,fig.height=3}
df %>% 
  ggplot() + 
  geom_dumbbell(aes(y = fct_reorder(name,real),
                    x = min, 
                    xend = max),
  colour = "#DADADA", size = 2,
  colour_x = "#E86767", colour_xend = "#2B8B0E")+
  geom_point(aes(x=real,y=fct_reorder(name,real)),size=4,color="dark orange",alpha=.7)+
  geom_text(aes(x=real,y=name,label = real,vjust = 2),size=2.6)+
  geom_text(aes(x=avg,y=name,label = avg,vjust = 2),size=2.6)+
  labs(
    title = "Comparativo do valor da ação com o valor estimado (+/- 2 desvios)",
    subtitle = "",
    x = "Valor da ação",
    y = "Empresas avaliadas"
  )+
  th+
  theme(legend.title = element_blank(),legend.position = "none")
```

O gráfico acima, gerado após a execução do algoritmo, resume os resultados obtidos. É possível concluir que o algoritmo consegue gerar valores próximos do atual valor das empresas analisadas. Embora a parte mais técnica de coleta de informações, como por exemplo calculo do beta ou calculo do custo da dívida, tenha tido menor relevância, o algoritmo permite escalabilidade, para analisar várias empresas de forma instantânea, bastando padronizar os dados de entrada.
