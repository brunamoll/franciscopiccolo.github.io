...
2. Coleta dos dados financeiros necessários para o cálculo do Fluxo de Caixa Operacional e da taxa de desconto (WACC).
3. Definição do período de projeção para o *valuation*.
4. Cálculo do valor residual.
5. Definição das variáveis que assumirão comportamento estocástico.
6. Definição da distribuição de probabilidade das variáveis estocásticas.
7. Análise e definição de correlações entre as variáveis estocásticas.
8. Criação do algoritmo para realização das iterações e geração do valor da empresa para cada iteração.

Ao final do processo de aplicação do algoritmo em várias empresas, será gerado um gráfico que irá mostrar, para cada empresa, o valor atual da ação e uma faixa de valor estimado pelo modelo, sendo esta faixa composta por um valor mínimo e máximo. O valor mínimo será o valor médio gerado pelo algoritmo menos 2 desvios padrão e para o valor máximo será adicionado ao valor médio 2 desvios padrão.
...
dados <- googlesheets::gs_title('dados_monografia') %>%
  googlesheets::gs_read(ws = 'alpargatas')

# Coletando as variáveis da planilha
n <- 5
ano <- dados[1,2:5]
stock_price <- as.numeric(dados[3,8])
stock_vol <- as.numeric(dados[2,8])
g <- as.numeric(dados[5,8])
rm <- rtriangle(n=10000,a=0.005,b=0.15,c=0.096)
rf <- rtriangle(n=10000,a=0.05,b=0.09,c=0.065)
beta <- as.numeric(dados[4,8])

ebitda <- as.numeric(dados[9,2:n])*1000
avg_ebitda <- mean(ebitda)
sd_ebitda <- sd(ebitda)

donp <- as.numeric(dados[10,2:n])*1000
avg_donp <- mean(donp)
sd_donp <- sd(donp)

invest <- as.numeric(dados[11,2:n])*1000
avg_invest <- mean(invest)
sd_invest <- sd(invest)

equity <- as.character(dados[14,2:n])
equity <- stringr::str_remove_all(equity,"%")
equity <- str_replace_all(equity,",",".")
equity <- as.numeric(equity)
avg_equity <- mean(equity)
sd_equity <- sd(equity)
equity <- rtriangle(n=10000,a=avg_equity-(2*sd_equity),b=avg_equity+(2*sd_equity),c=avg_equity)/100
debt <- 1- equity

kd <- as.character(dados[15,2:n])
kd <- stringr::str_remove_all(kd,"%")
kd <- str_replace_all(kd,",",".")
kd <- as.numeric(kd)
avg_kd <- mean(kd)
sd_kd <- sd(kd)
kd <- rtriangle(n=10000,a=avg_kd-(2*sd_kd),b=avg_kd+(2*sd_kd),c=avg_kd)

ke <- rf+(beta*(rm-rf))
wacc <- ((equity)*(kd)) + ((debt*ke))

# Valor residual
vr <- ((avg_ebitda)+avg_donp-avg_invest)/(wacc-g) # Valor Residual
...
for (i in 1:iter)     # criando loop do algoritmo
{
  saida[i] <- 
    (data.frame(              # criando data.frame para cada iteração
      ...
      dnp = rnorm(10, avg_donp, sd_donp),
      inv = rnorm(10, avg_invest, sd_invest),
      t = 1:10
    )%>%
      mutate(fco = ((ebda+dnp-inv)),
...
alpargatas_stock_est <-
  data.frame(
    name = "alpargatas",
    avg = mean(dados_saida$ev),
    max = mean(dados_saida$ev)+(2*(sd(dados_saida$ev))),
    min = mean(dados_saida$ev)-(2*(sd(dados_saida$ev))),
    real = stock_price
  )
...
# Base de dados
dados <- googlesheets::gs_title('dados_monografia') %>%
  googlesheets::gs_read(ws = 'carrefour')

# Coletando as variáveis da planilha e criando os parâmetros para distribuição normal
...
# Valor residual
vr <- ((avg_ebitda)+avg_donp-avg_invest)/((rltriangle(n=1, a=8, b=15, c=10)/100)-(rltriangle(n=1, a=2, b=5, c=4)/100)) # Valor Residual
...
for (i in 1:iter)     # criando loop do algoritmo
{
  saida[i] <- 
    (data.frame(              # criando data.frame para cada iteração
      ...
      mutate(fco = ((ebda+dnp-inv)),
             fcd = (fco / (1+(rltriangle(n=1, a=8, b=15, c=10)/100))^t) # distribuição triangular do wacc
      )%>% 
      summarize((sum(fcd)+vr)/(stock_vol))) # sumarizando os 10 anos e somando os valores com o valor residual
}
...
dplyr::union_all(
  ...
  ggplot() + 
  geom_dumbbell(aes(y = name,
                    x = min, 
                    xend = max),
  colour = "grey70", size = 3,
  colour_x = "#F7BC08", colour_xend = "#395B74")+
  geom_point(aes(x=real,y=name,color="dark orange",size=3))+
  theme(legend.title = element_blank(),legend.text = element_blank(),legend.position = "none",axis.title.y = element_blank())
